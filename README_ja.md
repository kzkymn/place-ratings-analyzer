...
`git pull`等でリポジトリを更新しても、動いているサーバーには反映されません。サーバーは
リポジトリのファイルではなくDockerイメージから起動しているためです。イメージを再ビルドして
...
ここまでの方法では、サーバを動かしているマシン自身からしかアクセスできません。
一方、認証を有効にした上でこのサーバをクラウド上にデプロイすれば、どこからでも
呼び出せるようになります。リモートMCPサーバとの接続にしか対応していない生成AI
...
必要です（認証、アクセス制御、公開状態のサーバーを維持し続けるコストなど）。
そのため、安全に運用する責任はすべてご自身にあります。上記の手順も、そのまま
使える完成品としてではなく、自分の状況に合わせて手を加えるための出発点として
扱ってください。
